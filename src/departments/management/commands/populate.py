...
from activities_class.models import TYPE_CHOICES as CLASS_TYPE_CHOICES
from activities_class.models import GlobalClassActivity
from activities_score.models import TYPE_CHOICES as SCORE_TYPE_CHOICES
from activities_score.models import GlobalScoreActivity
...
VALIDATE_ACTIVITIES = {
    ...
    'date'         : lambda datum        : datetime.strptime(datum, '%d.%m.%Y.'),
    ...
    def populate_activities(self):
        for subject in Subject.objects.all():
            ...
            for dictionary in dictionary_list:
                ...
                elif type_key in ('z', 'k', 's', 'dz', 'lz', 'prez'):
                ...
                if type_key in ('p', 'v', 'p+v'):
                    ...
                    self.stdout.write(self.style.SUCCESS('Nebodovna aktivnost "{}" kolegija "{}" uspješno stvorena'.format(final_dictionary['name'], subject.name)))
                else:
                    GlobalScoreActivity.objects.get_or_create(**final_dictionary)
                    self.stdout.write(self.style.SUCCESS('Bodovna ktivnost "{}" kolegija "{}" uspješno stvorena'.format(final_dictionary['name'], subject.name)))
